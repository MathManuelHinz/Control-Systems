...
\markeol{21}
\beginlecture{22}{10.07.2025}

Assumptions for this lecture (APP): \marginnote{Assumption on Parametrized Policy} 
...
\begin{algorithm}[H]
    ...
    \While{stopping criteria not fullfilled}
        \For{\(i=1,\dots K\)} 
            ...
            \State sample trajectory \(i\): \((x^i(0),u^i(0),x^i(1),c^i(1),u^i(1),\dots, u^i(T_i-1),x^i(T_i),c^i(T_i))\) \marginnote{ Like burn-in}
...
\begin{theorem}[Stochastic Gradient Descent (SGD) almost sure conv. for \(L\)-smooth functions]\label{thm:59}
    Let \(F:\R^d\to\R\) be a \(L\)-smooth with \(F_\star=\inf_{\theta\in\R^d}F(\theta)>-\infty\).
    It satisfies 
    \[\bE_{z\sim \mu_\theta}\left[\Vert \nabla f(\theta,z)-\bE\left[\nabla_\theta f(\theta,z)\right]\Vert^2\right]\leq c(1+(F(\theta)-F_\star))\] \marginnote{Like the probing signal previously}
    where \(\bE_{z\sim \mu_\theta}\left[\nabla_\theta f(\theta,z)\right]=\nabla F(\theta)\), this is called ergodic.
    Consider SGD with step 
    \begin{itemize}
        \item sample \(z_{+1}\) from \(\mu_\theta\) % Might be \theta_l
...
\begin{lemma}\label{lem:60} % what is B_\theta^2
...
\end{proof}
\markeol{22}
\beginlecture{23}{15.07.2025}
\begin{lemma}\label{lem:61} % TODO FIX: sum of fracs
...
\dhighlight{Critic:} value based algorithms that evaluate values of a \(Q\)-function. 

\dhighlight{Actor:} policy based algorithms that learn the policy by policy gradient.

Mixing both gives \dhighlight{actor-critic-methods}. This is were current research is being done mainly.

Estimating \(Q^{\phi^\theta}(x,u)\) by MC-sampling may have large variance, 
which influences the \dhighlight{convergence speed} in the gradient update.
Instead, use \(Q_w^{\phi^\theta}\) to approximate \(Q^{\phi^\theta}\), where \(w\) is a vector of unknowns.


\begin{proposition}\label{prop:63}
    If \(Q_w^{\phi^\theta}\) satisfies the \dhighlight{compatibility condition} with 
    the policy score function 
    \[\nabla_w Q_w^{\phi^\theta}(u;x)=\nabla_\theta \log \phi^\theta(u;x)\]
    and the \dhighlight{approximation property}
...
\begin{definition}\label{def:64}
    The \dhighlight{advantage} of a policy is defined as 
...
\section{control theory: roughly 1/4}
\begin{itemize}
