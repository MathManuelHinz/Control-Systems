...
%TODO based on dynamic programming and optimal control Vol 2 fourth edition 6.1 / 6.2
% the most mathy, but not mathy enough for us :D The start is from his book

\section{Monte Carlo Sampling / Simulation}
%TODO: today focuses on value functions

\dhighlight{Aim:} Generate trajectories, use observed states,
actions and costs. Use that to directly estimate \(V^\phi, J^\phi\). % Policy evaluation, both the same
...
This solves approximately \(J^\phi=T^\phi J^\phi\). In the terminology of Berzicas %TODO: FIX
...
\end{align*}
\marginnote{this is now actually computable}
...
%the minimization with c(i,m) is the key idea.
\markeol{19}
